...
def runScript(delay, SCRIPT_NAME):
    ...
    k.type_string('/home/antonbilbaeno/proj/st/pattern/' + SCRIPT_NAME)
...
def via_noNet(x, y, drill=10, diameter='0', shape='round', layer1=1, layer2=16):
    setDrill(drill, 'mil')
...
def labelBoard():
    ...
    pad_string = 'R: ' + str(row) + ', C: ' + str(col) + ', SIZE: ' + str(int(TPAD_H))
    text(pad_string, float(-boardW/2+0.5), float(boardH/2-3), 1)
    pad_pitch_string = 'P PITCH- X: ' + str(tPadPx) + ', Y: ' + str(tPadPy)
    text(pad_pitch_string, float(-boardW/2+0.5), float(boardH/2-4.5), 1)
    trace_string = 'T- W: ' + str(TRACE_W) + ', P: ' + str(TRACE_P)
    text(trace_string, float(-boardW/2+0.5), float(boardH/2-6), 1)
    ...
    for gridIndex, grid in enumerate(allPtPairs):
        ...
        if gridIndex >= 0 and gridIndex <= 3:
            ...
            for pair in sortedGrid:
                ...
                drawPolygon(pair['tx'], pair['ty'], TPAD_W, TPAD_H)
                if PACKED == False:
                    via_noNet(pair['tx'], pair['ty'], 10, 0, 'round', 1, 16)
